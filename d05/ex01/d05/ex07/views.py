...
def display(request):
    ...
    for r in result:
        tr.append(e.Tr(e.Td([e.Text(str(r.episode_nb) ),
         ...
         e.Text(str(r.created) ),
         e.Text(str(r.updated) )
         ]
        )))
    table = e.Table(tr)
...
def update(request):
    ...
    if request.method == 'POST':
        if 'title' in  request.POST:
            d = Movies.objects.get(title=str(request.POST['title']))
            d.opening_crawl = str(request.POST['opening_crawl'])
            d.save()
    m = Movies()
    result = Movies.objects.all()
    ...
    for r in result:
        content.append(
        e.Option(attr={'value':str(r.title)})
        )
        content.append(
        e.Text(str(r.title))
        )
